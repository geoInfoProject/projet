...
public interface DemandeRepository extends JpaRepository<Demande, Long> {
	...
	@Query("SELECT new com.GestionDossiers.demande_autorisation.dto.DemandeDTO(" +
		       "d.id, d.dateDemande, d.x, d.y, " +
		       "COALESCE(c.libelle, ''), cl.cin, cl.nom, cl.prenom, " +
		       "s.statut, a.type) " +
		       "FROM Demande d " +
		       "LEFT JOIN d.commune c " +
		       "LEFT JOIN d.client cl " +
		       "LEFT JOIN d.status s " +
		       "LEFT JOIN d.autorisation a")
    ...
    @Query("SELECT d FROM Demande d " +
    	       "LEFT JOIN d.commune c " +
    	       "WHERE d.client.id = :clientId AND " +
    	       "(:statut IS NULL OR d.status.statut = :statut) AND " +
    	       "(:type IS NULL OR d.autorisation.type = :type) AND " +
    	       "(:commune IS NULL OR c.libelle = :commune)")
