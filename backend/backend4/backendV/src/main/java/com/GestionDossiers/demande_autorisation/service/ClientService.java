...
public class ClientService {
    ...
    public int authentifierClient(String email, String motDePasse) {
        Client client = clientRepository.findByEmail(email);
        if (client != null && client.getMotDePasse().equals(motDePasse)) {
            return client.getId(); // Retourner l'ID du client authentifié
        }
        return -1; // Indique un échec de l'authentification
    }
}
